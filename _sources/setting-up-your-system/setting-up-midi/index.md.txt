# Configuring MIDI
[MIDI]{.dfn} is a way to describe musical performances and to control
...
| Linux   | [ALSA MIDI]{.dfn} is the standard MIDI framework on Linux systems. |
| Windows | [MME]{.dfn} is the standard MIDI framework on Windows systems.     |

[JACK]{.dfn} is an alternate audio system which Ardour can utilize for
both audio and MIDI. JACK is used to route audio between independent
applications, and is now considered an advanced use case which is not
recommended for most users. Users with a need to use JACK for audio
routing should consult the latest documentation at the [JACK
