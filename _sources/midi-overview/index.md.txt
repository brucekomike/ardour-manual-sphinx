...
-   MIDI, just like audio, exists in regions. MIDI regions behave like
    ...
    MIDI region content (the notes), but never both at the same time.
    The [e]{.kbd} key (by default) sets [Internal
    Edit](@@toolbox#edit-internal) Mode, which allows the editing of
    MIDI data in a given region.
...
-   Controllers (CC data) can be set to discrete or continuous modes;
    the latter will linearly interpolate between control points and send
    additional data.
...
-   By default, copying a MIDI region creates a [deep link]{.dfn}---both
    ...
    \> Unlink from other copies]{.kbd .menu} from the region context
    menu, after which the selected region(s) will have their own copies
    of *only* the data that they visually display on screen. The region
    will no longer be trimmable back to its original length after an
    Unlink operation, and the operation cannot be undone.
