...
-   ffmpeg, ffprobe (used to import/export video, extract soundtracks
    and query video information and encode mp3 for export):
...
-   Ardour launches `XJREMOTE` (environment variable, default
    \'xjremote\' which comes with xjadeo).
-   Either use a custom shell script that ssh\'es into the remote box
    and launches/controls xjadeo there, selects the sync-source and
    passes though communication between ardour ⇔ xjadeo via ssh (xjadeo
    is launched stopped with the session).
-   ...or override xjremote\'s behavior---instead of IPC with a local
    running xjadeo-process, using OSC for example. Xjadeo would run
    permanently and Ardour will just tell it to load files and set
    offsets via OSC. See
