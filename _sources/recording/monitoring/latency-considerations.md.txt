...
A/D/A conversion time is objectionable. However even acoustic
instruments such as the piano can have approximately 3 ms of latency,
due to the time the sound takes to travel from the instrument to the
musician\'s ears. Latency below 5 ms should be suitable for a
professional recording setup. Because 2 ms are already used in the A/D/A
process, extremely low [buffer sizes]{.dfn} must be used in the
workstation I/O setup to keep the overall latency below 5ms. Not all
[computer audio systems](@@the-right-computer-system-for-digital-audio)
