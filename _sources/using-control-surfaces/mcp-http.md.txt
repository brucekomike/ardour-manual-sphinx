...
The [MCP]{.dfn} HTTP control surface is an experimental server that lets
an MCP-capable assistant inspect and control a running Ardour session.
...
-   Region inspection, move, copy, resize, split, gain, and
    normalization.
-   MIDI region creation, note insertion, note listing, note editing,
    and bulk JSON import or export.
...
-   Create a bus called Vocal Verb and add post-fader sends from the
    vocal tracks to it.
...
-   In bar 4, add solid quarter-note chords in the key of F on each
    beat.
...
-   Most edit operations identify tracks, plugins, regions, and markers
    by explicit IDs or names.
-   Selected-region targeting is not implemented.
-   MCP clients and models vary in how well they use tool descriptions
    and structured return data.
