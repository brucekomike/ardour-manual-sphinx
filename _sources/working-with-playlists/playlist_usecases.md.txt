...
be done by creating a new track, applying the original track\'s
playlist, and then applying effects to both tracks independently.
...
Using Playlists for [takes]{.dfn} is a good solution when one needs the
ability to edit individual takes, and select between them.

Each time a new take is started, a new playlist should be created with
[p \> New]{.kbd .menu}. Thus, later, any previous or later takes can be
selected as desired.
...
-   by creating a new track to assemble the final version, and \"cherry
    picking\" from the playlists in the original track by copying
    regions over as required
