...
-   If the [Edit Point](@@edit-point-control) is [mouse]{.kbd .menu},
    then
...
-   For all other Edit Points
    -   use the selected regions *and* those that are both under the
        edit position *and* on a selected track, or on a track which is
        in the same active edit-enabled route group as a selected
        region.
