# Snapshots
A [snapshot]{.dfn} is a backup of the current state of a session. It
differs from a simple save by allowing multiple versions of the session
to exist. You can think of a snapshot as either a \"frozen\" version of
the session at a certain point in time, or as a starting point for a
different approach to editing and mixing within the same session.
...
This is accomplished by using either of the [Session \> Snapshot]{.kbd
.menu} menus. A small dialog will appear, allowing to enter a name for
...
Creating a snapshot does **not** modify the session, nor does it save
...
Switching to an existing snapshot is done by navigating the [Snapshot
List](@@the-snapshot-list) and clicking the the name of the desired
...
The browser in the \"Open Session\" dialog will show an expander arrow
for sessions that have more than one session file (i.e. snapshots)
present. Clicking on it shows the list, and then clicking on the name of
the snapshot loads it.
