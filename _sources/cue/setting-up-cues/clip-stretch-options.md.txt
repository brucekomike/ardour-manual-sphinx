...
[minibpm](https://github.com/breakfastquay/minibpm) is used to analyze
and detect the file\'s BPM.

After tempo is estimated, the clip is time-stretched to match the
session\'s tempo map. This means that should session\'s tempo change
over time (in either ramped or constant mode), all audio clips will be
...
-   [Crisp]{.dfn} works best for sounds with fast onset like drums and
    percussion
-   [Smooth]{.dfn} is best used for sustained notes like pads
-   [Mixed]{.dfn} is for anything in between
...
tempo is 90bpm, stretching the clip to match session\'s tempo will make
it sound faster that it originally is.
...
from 180bpm to 120bpm will make the clip sound slower than it originally
is.
